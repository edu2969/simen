...
ProcessAssistanceHH = function (id) {
  ...
  const esViernes = moment(date).get("day") == 5;

  var marcas = reg.marcas ? reg.marcas : [];

  var lastHHNormal = reg.hhNormal ? reg.hhNormal : (esViernes ? 7.5 : 9),
  ...
  if (!reg.vacacion) {
    if (!TipoJornada(date) && date.getDay() != 0 && date.getDay() != 6) {
      if (msCounter < (esViernes ? 7.5 : 9) * 60 * 60 * 3600) {
        reg.hhExt50 = Math.floor(((msCounter - 9 * 60 * 60 * 3600) / (60 * 60 * 3600)) * 2) / 2;
      } 
      reg.hhNormal = Math.floor(msCounter / (60 * 60 * 3600));        
  ...
  var doc = !reg.vacacion ? {
    $set: {
      hhNormal: reg.hhNormal ? reg.hhNormal : (esViernes ? 7.5 : 9),
...
export const TIPOS_JORNADAS = {
  FERIADO_LEGAL: 1,
  COMPENSACION_VACACIONES: 2,
  MEDIA_JORNADA: 3
}
