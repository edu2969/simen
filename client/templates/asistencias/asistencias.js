...
Template.asistencias.events({
  ...
  'click .btn-view': function (e) {
    ...
    AddNav('/asistencias');
    Router.go('/asistenciaDetalle/' + Number(bioId) + '/' + periodo);
  },
...
function loadColaboratorsChart(idUser) {
  ...
  svg.append("rect")
    ...
    .on("click", function () {
      ...
      AddNav('/asistencias');
      Router.go("/asistenciaDetalle/" + bioId + "/" + (d.date.getMonth() + 1) + '/' + d.date.getFullYear());
    });
...
function loadTopHHExtBar(date) {
  ...
  function clickbar(d) {
    ...
    AddNav('/asistencias');
    Router.go("/asistenciaDetalle/" + bioId + "/" + (date.getMonth() + 1) + '/' + date.getFullYear());
  }
}
