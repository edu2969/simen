...
PDFCotizaciones = function (cId) {
  ...
  doc
    .lineJoin("miter")
    .rect(50, top + hsize * 0, col48 * 48, hsize)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50, top + hsize * 1, col48 * 48, hsize)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50, top + hsize * 2, col48 * 48, hsize)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50, top + hsize * 3, col48 * 48, hsize)
    .stroke();
  if (!_.isEmpty(cliente.contactoNombreCC)) {
    doc
      .lineJoin("miter")
      .rect(50, top + hsize * 4, col48 * 48, hsize)
      .stroke();
  }
  ...
  doc.text(
    "At. " + cliente.atencion,
    50 + textPadding,
    top + textPadding + hsize * 1
  );
  if (!_.isEmpty(cliente.contactoNombreCC)) {
    doc.text(
      "CC. " + cliente.atencionCC,
      50 + textPadding,
      top + textPadding + hsize * 2
    );
  }
  doc.text(
    "De",
    50 + textPadding,
    top + textPadding + hsize * (_.isEmpty(cliente.contactoNombreCC) ? 2 : 3)
  );
  doc.text(
    "Ref.",
    50 + textPadding,
    top + textPadding + hsize * (_.isEmpty(cliente.contactoNombreCC) ? 3 : 4)
  );
  ...
  if (!_.isEmpty(cliente.contactoNombreCC)) {
    doc.text(
      ": " + cliente.contactoNombreCC,
      100,
      top + textPadding + hsize * 2
    );
  }
  doc.text(
    ": Diego Mendoza. Cel +56 9 9886 7623",
    100,
    top + textPadding + hsize * (_.isEmpty(cliente.contactoNombreCC) ? 2 : 3)
  );
  doc.text(
  ...
  doc.text(
    ...
    top +
      textPadding +
      hsize * (_.isEmpty(cliente.contactoNombreCC) ? 4 : 5) +
      15
  );
  doc.text(
    ...
    top +
      textPadding +
      hsize * (_.isEmpty(cliente.contactoNombreCC) ? 5 : 6) +
      8
  );
  ...
  doc.text(
    cotizacion.titulo,
    50,
    top + textPadding + hsize * (_.isEmpty(cliente.contactoNombreCC) ? 6 : 7)
  );
  top =
    top + textPadding + hsize * (_.isEmpty(cliente.contactoNombreCC) ? 7 : 8);

  doc.font(fontNormal);
  doc.lineWidth(1);
  doc
    .lineJoin("miter")
    .rect(50, top, col48 * 2, hsize * 1.5)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + col48 * 2, top, col48 * 4, hsize * 1.5)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + col48 * 6, top, col48 * 5, hsize * 1.5)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + col48 * 11, top, col48 * 21, hsize * 1.5)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + col48 * 32, top, col48 * 8, hsize * 1.5)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + col48 * 40, top, col48 * 8, hsize * 1.5)
    .stroke();
  ...
  items.forEach(function (item) {
    ...
    var titulo = item.descripcion.includes("#")
      ? item.descripcion.split("#")[0]
      : false;
    var descripcion = item.descripcion.includes("#")
    ...
    var alturaTotal = 0;
    var topCeldas = top + hsize * 0.6 * cantidad - textPadding * 2 + 1;
    var lineaSuperior = topCeldas;
    for (var i = 0; i < lineas.length; i++) {
      var uc = (i == 0 ? lineas[i] : lineas[i].replace("- ", "")).replace(
        /[^A-Z,0-9]/g,
        ""
      ).length;
      var lc = (i == 0 ? lineas[i] : lineas[i].replace("- ", "")).length - uc;
      var relacion =
        (uc * 6.2 + lc * 5.2 - textPadding * 2 - (i == 0 ? 0 : 20)) /
        (col48 * 21 - textPadding * 2 - (i == 0 ? 0 : 20));
      var altura = 1 + Math.floor(relacion);
      doc.font(i == 0 ? fontBold : fontNormal).fontSize(10);
      doc.text(
        i == 0 ? lineas[i] : lineas[i].replace("- ", ""),
        (i == 0 ? 50 : 70) + textPadding + col48 * 11,
        lineaSuperior + (lineas.length > 1 ? 4 : textPadding),
        {
          align: "left",
      ...
      lineaSuperior =
        lineaSuperior + altura * hsize * 0.6 + textPadding * (i == 0 ? 1 : 0);
    ...
    doc.text(
      item.cantidad,
      44 + textPadding + col48 * 2,
      topCeldas + textPadding,
      {
        align: "center",
        width: col48 * 4,
      }
    ...
    doc.text(
      subcategoria ? subcategoria.codigo : "XXX",
      50 + textPadding + col48 * 6,
      topCeldas + textPadding
    );

    doc.text(
      item.moneda,
      50 + textPadding + col48 * 32,
      topCeldas + textPadding
    );

    doc.text(
      MaskPrice(item.valor, ""),
      50 + textPadding + col48 * 32,
      topCeldas + textPadding,
      {
        align: "right",
    ...
    doc.text(moneda, 50 + textPadding + col48 * 40, topCeldas + textPadding);

    doc.text(
      MaskPrice(item.valor * item.cantidad, item.moneda),
      50 + textPadding + col48 * 40,
      topCeldas + textPadding,
      {
        align: "right",
        width: col48 * 8 - textPadding * 2,
    ...
    doc
      ...
      .rect(
        ...
        topCeldas,
        col48 * 2,
        alturaTotal * hsize * 0.6 + textPadding * 2
      )
      .stroke();
    doc
      .lineJoin("miter")
    ...
    top = topCeldas + alturaTotal * hsize * 0.6 + textPadding * 2;
    ...
    if (top > 800) {
      ...
      doc
        .rect(50 + col48 * 1, top, col48 * 2 - 1, hsize - 1)
        .fill("#22829f", "even-odd");
      doc
        .rect(50 + col48 * 3, top, col48 * 4 - 1, hsize - 1)
        .fill("#22829f", "even-odd");
      doc
        .rect(50 + col48 * 7, top, col48 * 9 - 1, hsize - 1)
        .fill("#22829f", "even-odd");
      doc
        .rect(50 + col48 * 16, top, col48 * 4 - 2, hsize - 1)
        .fill("#22829f", "even-odd");
      doc
        .rect(50 + col48 * 20, top, col48 * 4, hsize - 1)
        .fill("#22829f", "even-odd");
  ...
  doc
    .lineJoin("miter")
    .rect(50, top + hsize * (cantidad + 0), col48 * 40, hsize)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50, top + hsize * (cantidad + 1), col48 * 40, hsize)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50, top + hsize * (cantidad + 2), col48 * 40, hsize)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + col48 * 40, top + hsize * (cantidad + 0), col48 * 8, hsize)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + col48 * 40, top + hsize * (cantidad + 1), col48 * 8, hsize)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + col48 * 40, top + hsize * (cantidad + 2), col48 * 8, hsize)
    .stroke();
  ...
  doc.text(
    moneda,
    50 + col48 * 40 + textPadding,
    top + hsize * (cantidad + 0) + textPadding / 2
  );
  doc.text(
    MaskPrice(total, ""),
    50 + col48 * 40 + textPadding,
  ...
  doc.text(
    moneda,
    50 + col48 * 40 + textPadding,
    top + hsize * (cantidad + 1) + textPadding / 2
  );
  doc.text(
    MaskPrice(iva, ""),
    50 + col48 * 40 + textPadding / 2,
    top + hsize * (cantidad + 1) + textPadding / 2,
    {
      align: "right",
      width: col48 * 8 - textPadding * 2,
  ...
  doc.text(
    moneda,
    50 + col48 * 40 + textPadding,
    top + hsize * (cantidad + 2) + textPadding / 2
  );
  doc.text(
    MaskPrice(conIva, ""),
    50 + col48 * 40 + textPadding,
  ...
  doc
    .lineJoin("miter")
    .rect(46, tc + 20, 512, 20)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(46, tc + 40, 512, 60)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(46, tc + 100, 512, 20)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(46, tc + 120, 512, 20)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(46, tc + 140, 512, 40)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(46, tc + 180, 512, 20)
    .stroke();
  ...
  doc
    ...
    .font(fontBold)
    .fillColor(
      empresaOrigen.identificador.includes("tyt") ? "#084355" : "#3C3C3B"
    );
  ...
  doc.text(
    cotizacion.garantia +
      " a partir de fecha de entrega, por fallas " +
      "de fabricación imputables a fabricante. " +
      "Esta caduca automáticamente si los equipos " +
      "son intervenidos durante este periodo o no se " +
      "respetan las indicaciones del fabricante.",
    170,
    topeTmp + 44
  );
  doc.text(
    "10 dias a partir de fecha indicada en la presente cotización",
    170,
    topeTmp + 84
  );
  doc.text(
    "En caso de ser favorecidos con vuestro pedido, favor " +
      "emitir su Orden de Compra a nombre de ",
    50,
    topeTmp + 142
  );
  doc.text(
    "No se aceptarán multas ni retenciones de ningún tipo" +
      " sin acuerdo previo entre las partes.",
    170,
    topeTmp + 224
  );

  doc.text(
    "Los servicios de garantía serán prestados en laboratorios de " +
      empresaOrigen.razon +
      " siendo los cargos de traslado, " +
      "hacia y desde el sitio indicado, de parte del Cliente",
    170,
    topeTmp + 254
  );

  doc.text(
    "La presente oferta no incluye servicios de montaje de ningún tipo, " +
      "a excepción de los expresamente indicados en la oferta. " +
      "Los costos de puesta en servicio del equipamiento cotizado " +
      "serán cargados al cliente de acuerdo a los procedimientos que " +
      empresaOrigen.razon +
  ...
  doc.text(
    "Una vez aceptada en conformidad la orden de compra por parte de " +
      empresaOrigen.razon +
      ", cualquier cambio, anulación, cancelación " +
      "o devolución de productos, por parte del cliente, podrá generar " +
      "cargos, los cuales deberán ser asumidos por el cliente.",
    170,
    topeTmp + 332
  );
  ...
  doc.text(
    cotizacion.cuenta == "1" ? empresaOrigen.banco1 : empresaOrigen.banco2,
    50,
    tcu + 12,
    {
      width: 500,
  ...
  doc.font(fontNormal);
  doc.text(
    "Cuenta Corriente N° " + cotizacion.cuenta == "1"
      ? empresaOrigen.cuenta1
      : empresaOrigen.cuenta2,
    50,
    tcu + 26,
    {
      width: 500,
  ...
  doc.text(
    "Fono: " +
      empresaOrigen.telefonos +
      " e-mail: " +
      empresaOrigen.contactoEmail,
    50,
    topeTmp + 206,
    {
      width: 500,
      ...
      for (var index = 0; index < marcas.length; index++) {
        ...
        if (!marcas[index].invalida) {
          ...
          if (
            ...
            marcas[0].ms < 9 * 60 * 60 * 3600 + 10 * 60 * 3600
          ) {
            item.isLate = true;
            stats.lateDays++;
          }
        ...
        doc.text(
          item.glyphicon + item.time,
          x + 33 + (index % 4) * 45,
          y + tp + Math.floor(index / 4) * 19
        ) + 4;
      }
...
PDFLibroMenor = function (params) {
  ...
  doc
    .lineJoin("miter")
    .rect(50, top - altura, 6 * col48, altura)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + 6 * col48, top - altura, 3 * col48, altura)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + 9 * col48, top - altura, 12 * col48, altura)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + 21 * col48, top - altura, 9 * col48, altura)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + 30 * col48, top - altura, 9 * col48, altura)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + 39 * col48, top - altura, 9 * col48, altura)
    .stroke();
  ...
  cotizaciones.forEach(function (c) {
    ...
    doc.text(
      moment(c.fecha).get("date"),
      50 + tp + 6 * col48,
      top + altura * indice + tp
    );
    doc.text(empresa.razon, 50 + tp + 9 * col48, top + altura * indice + tp);
    doc.text(
      MaskPrice(c.total * 1.19, "$"),
      50 + tp + 21 * col48,
      top + altura * indice + tp,
      {
        width: 9 * col48 - 2 * tp,
    ...
    doc
      .lineJoin("miter")
      .rect(50, top + altura * indice, 6 * col48, altura)
      .stroke();
    doc
      .lineJoin("miter")
      .rect(50 + 6 * col48, top + altura * indice, 3 * col48, altura)
      .stroke();
    doc
      .lineJoin("miter")
      .rect(50 + 9 * col48, top + altura * indice, 12 * col48, altura)
      .stroke();
    doc
      .lineJoin("miter")
      .rect(50 + 21 * col48, top + altura * indice, 9 * col48, altura)
      .stroke();
    doc
      .lineJoin("miter")
      .rect(50 + 30 * col48, top + altura * indice, 9 * col48, altura)
      .stroke();
    doc
      .lineJoin("miter")
      .rect(50 + 39 * col48, top + altura * indice, 9 * col48, altura)
      .stroke();
  ...
  doc
    .lineJoin("miter")
    .rect(50, top + altura * indice, 21 * col48, altura)
    .stroke();
  doc
    .lineJoin("miter")
    .rect(50 + 21 * col48, top + altura * indice, 9 * col48, altura)
...
Meteor.methods({
  ...
  ActualizarUsuario: function (userId, doc) {
    if (!userId) {
      ...
      Meteor.users.update(
        ...
        {
          $set: {
            "profile.nombres": doc.nombres,
            "profile.apellidos": doc.apellidos,
            "profile.rut": doc.rut,
  ...
  ProcesarAsistencia: function (bioId, cadena) {
    ...
    var msmarcacion =
      fecha.hours() * 60 * 60 * 3600 + fecha.minutes() * 60 * 3600;
